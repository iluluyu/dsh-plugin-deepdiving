	...
	factory: (require) => {
		...
		const COPY = {
			zh: {
				...
				speedConstant: "恒定",
				speedFollow: "跟随生成速度",
				followHint: "0.5×–3×：1× 为官网节奏，输出越快越快。",
				multLabel: "速度倍速",
				multHint: "1× 为官网节奏。",
				forceLabel: "减弱动态时仍流动",
			...
			en: {
				...
				followHint: "0.5×–3×: 1× is the official cadence; faster with output.",
				multLabel: "Speed",
				multHint: "1× is the official cadence.",
		...
		/* Ticks scale: |-|-|-|-| with numeric labels under each tick;
		 * clicking a tick selects it. 3x sits left, 0.5x right (fast to
		 * slow, matching a volume-style right-to-left decrease). */
		.dv-ticks {
		...
		.dv-tick {
			width: 26px; display: flex; flex-direction: column; align-items: center;
			padding-top: 12px; position: relative; border: none; background: none;
			font: inherit; cursor: pointer;
		}
		.dv-tick::before {
			/* the tick mark: one segment of the | - | - | ruler */
			content: ""; position: absolute; top: 2px; left: 0; right: 0;
			height: 8px;
			border-left: 1.5px solid var(--dsw-alias-label-dimmed, rgba(0,0,0,0.2));
		}
		.dv-tick::after {
			/* the crossbar: connects ticks into |-| segments */
			content: ""; position: absolute; top: 9px; left: 0; right: 0;
			height: 1.5px;
			background: var(--dsw-alias-label-dimmed, rgba(0,0,0,0.2));
		}
		.dv-tick:first-child::before { left: 50%; }
		.dv-tick:last-child::before { right: 50%; }
		.dv-tick:first-child::after { left: 50%; border-radius: 0 0 0 2px; }
		.dv-tick:last-child::after { right: 50%; border-radius: 0 0 2px 0; }
		body[data-ds-dark-theme] .dv-tick::before {
			border-color: var(--dsw-alias-label-dimmed, rgba(255,255,255,0.25));
		}
		body[data-ds-dark-theme] .dv-tick::after {
			background: var(--dsw-alias-label-dimmed, rgba(255,255,255,0.25));
		}
		/* selected tick: filled segment + brand label */
		.dv-tick.dv-on::before {
			border-left-color: var(--dsw-static-deepseek-500, rgb(65 118 230));
		...
		.dv-tickNum {
			font-size: 11px; line-height: 14px; font-variant-numeric: tabular-nums;
			color: var(--dsw-alias-label-tertiary);
			margin-top: 2px;
		}
		.dv-tick.dv-on .dv-tickNum {
			color: var(--dsw-static-deepseek-500, rgb(65 118 230));
			font-weight: 600;
		}
		.dv-tick:focus-visible {
			outline: 2px solid var(--dsw-alias-brand-primary); outline-offset: 2px;
			border-radius: 4px;
		}
		...
		/** The ticks scale: |-|-|-|-| with labels 3 2 1.5 1 0.5 (fast to
		 * slow, left to right). Each tick is a button; clicking selects that
		 * multiplier.
		 * @param props - value (multiplier), ticks (label pairs), onChange.
		 */
		function TicksScale({ value, ticks, onChange }) {
			return h("div", { className: "dv-ticks", role: "group" },
				ticks.map(([mult, label]) =>
					h("button", {
						key: String(mult), type: "button",
						className: "dv-tick" + (mult === value ? " dv-on" : ""),
						"aria-pressed": mult === value,
						"aria-label": String(label) + "x",
						onClick: () => { onChange(mult); },
					},
						h("span", { className: "dv-tickNum" }, label),
					),
				),
			);
		}
		...
		function DeepdivingCard({ copy }) {
			...
			const [mult, setMult] = useState(readMult);
			...
			const applyMult = (m) => {
				setMult(m); write(LS_MULT, String(m));
				window.dispatchEvent(new CustomEvent("dsh-deepdiving:prefs", { detail: { mult: m } }));
			};
			const onSpeed = (id) => {
					...
					h("div", { className: "dv-field" },
						...
						speed === "constant"
							? h("div", { className: "dv-field" },
								...
								h(TicksScale, { value: mult, ticks: TICKS, onChange: applyMult }),
								h("p", { className: "dv-hint" }, t("multHint")),
							)
							: h("p", { className: "dv-hint" }, t("followHint")),
					),
					h("div", { className: "dv-field" },
		...
		function PaceSampler() {
			...
			const tick = () => {
				...
				/* Follow curve (1x anchor): 0 -> 12s, 150cps(~50tok/s) -> 4s
				 * official cadence, 750cps ceiling -> 2s. The user multiplier
				 * scales the cadence: duration = base / mult, so 3x maps the
				 * anchor to 1.33s and the ceiling to 0.67s, 0.5x doubles them. */
				let dur;
				if (rate <= 150) {
					dur = 12 - (rate / 150) * 8;
				} else {
				...
				const mult = readMult();
				if (mult !== 1) dur /= mult;
				/* 0.1s grain (visually indistinguishable, keeps 3x honest:
				 * 4s/3 -> 1.3s not the 1.5s a 0.5s grain would round to) */
				dur = Math.max(0.7, Math.round(dur * 10) / 10);
				if (dur !== lastDur) {
					lastDur = dur;
		...
		function apply(ctx) {
			ctx.effect(() => {
				...
				const applyConstant = () => {
					if (readSpeedMode() !== "follow") {
						/* constant mode: duration = base / multiplier (1x = 4s
						 * official cadence; 3x -> 1.33s, 0.5x -> 8s) */
						const dur = BASE_S / readMult();
						document.documentElement.style.setProperty("--dv-dur", dur + "s");
					}
				};
				const syncSampler = () => {
					if (readSpeedMode() === "follow") { sampler.start(); }
					else { sampler.stop(); applyConstant(); }
				};
