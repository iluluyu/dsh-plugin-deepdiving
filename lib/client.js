...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const DEFAULTS = { speedMode: "constant", mult: 1, forceFlow: true };
		...
		function readSpeedMode() {
			try { const v = window.localStorage.getItem(LS_SPEED); return v === "follow" ? "follow" : "constant"; } catch { return DEFAULTS.speedMode; }
		}
		function readMult() {
			try {
