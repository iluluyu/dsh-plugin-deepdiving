...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const COPY = {
			zh: {
				title: "Deep diving 水流",
				description: "“Deep diving…” 状态条的流光动画。",
				expand: "展开", collapse: "收起",
				speedLabel: "流动速度",
				speedHint: "跟随模式下，输出越快水流越快。",
				speedConstant: "恒定（6 秒）",
				speedFollow: "跟随生成速度",
				forceLabel: "减弱动态时仍流动",
				forceHint: "忽略浏览器的减弱动态请求。",
			},
			en: {
				title: "Deep diving water flow",
				description: "The flowing light on the “Deep diving…” status line.",
				expand: "Expand", collapse: "Collapse",
				speedLabel: "Flow speed",
				speedHint: "Follow mode: the water speeds up with output.",
				speedConstant: "Constant (6s)",
		...
		function DeepdivingCard({ copy }) {
			...
			const onSpeed = (e) => {
				const mode = e.target.value === "follow" ? "follow" : "constant";
				setSpeed(mode); write(LS_SPEED, mode);
			...
			return h("li", { className: "dv-card" + (open ? " dv-open" : "") },
				...
				open ? h("div", { className: "dv-body" },
					h("div", { className: "dv-field" },
						...
						h("select", {
							id: "dv-speed", className: "dv-select", value: speed, onChange: onSpeed,
						},
							h("option", { value: "constant" }, t("speedConstant")),
							h("option", { value: "follow" }, t("speedFollow")),
						),
		...
		function PaceSampler() {
			...
			const tick = () => {
				...
				// Linear map: 0 cps -> 12s, >=250 cps -> 2.5s.
				const dur = Math.round((12 - Math.min(rate / 250, 1) * 9.5) * 2) / 2;
				if (dur !== lastDur) {
					lastDur = dur;
