	...
	factory: (require) => {
		...
		function DeepdivingCard({ copy }) {
			...
			const applyMult = (m) => {
				 ...
				 * syncSampler -> applyConstant reads the mode and the stored mult. */
		...
		function apply(ctx) {
			...
			ctx.effect(() => {
				...
				// rc.7: settings.plugin.item became keyed by host settings namespace
			// (for official cards like bash/agent-loop). Third-party cards live as
			// their own tab under settings.plugins.tab (list slot): "feature plugins
			// contribute pages without competing for Settings nav rows".
			const unregister = ctx.slots.register({
				name: "settings.plugins.tab",
				id: "deepdiving",
				order: 100,
				label: () => copyFor(ctx.get("locale") === undefined ? "zh" : ctx.get("locale").getSnapshot().active).title,
			}, card);
